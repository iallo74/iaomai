...
function en(txt){
	return(LZString.compressToBase64(JSON.stringify(txt)))
}
function qq(txt){
	delete txt["materials"];
	delete txt["object"]["material"];
	return en(txt);
}
function comprA(str){
	str = str.replace('data:image/jpeg;base64,','');
	txt2 = str.replace(/([A]{4,})/g, function ($0, $1) {
		return '[A'+$1.length+']';
	});	
	return txt2;
}
function decomprA(str){
	txt2 = str.replace(/(\[A)([0-9]+)(\])/g, function ($0, $1, $2, $3) {
		return new Array($2 * 1 + 1).join("A");
	});
	return txt2;
}