...
var PAZIENTI_TRATTAMENTI = {
		...
		if(PAZIENTI.idCL>-1){
			...
			for(c in PAZIENTI.cicli){
				...
				if(PAZIENTI.cicli[c].Tipo == 'C' || PAZIENTI.cicli[c].Tipo == 'V'){
					...
					HTML += '	<span>'+htmlEntities(TXT("SchedaAnamnesi"))+'</span>' +
							'</div>';
					}
					HTML += HTMLProvv +
							'</div></div>';
	...
	car_trattamento: function( Q_idTratt, btn, LabelCiclo, an, idCiclo, trasforma ){ // scheda del trattamento
		...
		CONFIRM.vis(	TXT("UscireSenzaSalvare"),
			...
			if(Q_idTratt>-1 || an){
				if(TipoTrattamento=='A')titoloDef=TXT("SchedaAnamnesi");
		...
		if(PAZIENTI.idCL>-1){
			...
			if(TipoTrattamento=='A'){ // verifico non ci sia già LabelCiclo
				...
				if(PAZIENTI.sintomiEliminati.length){
					...
					for(d in PAZIENTI.sintomiEliminati){
						for(t in TRS){
							var mod = false;
							var sint = JSON.parse(__(TRS[t].sintomi,[]));
							for(s in sint){
							...
							if(mod)TRS[t].sintomi = JSON.stringify(sint);
						}
					}
					PAZIENTI.sintomiEliminati = [];
				}
			...
			for(i in TRS){
				...
				if(TR.Cancellato==0 && TR.LabelCiclo==LabelCiclo){
					...
					if(TipoTrattamento!='A'){
						...
						AnamnesiDiagnosiOccidentale=TestoTrattamento.AnamnesiDiagnosiOccidentale;
						AnamnesiDiagnosiMTC=TestoTrattamento.AnamnesiDiagnosiMTC;
						if(AnamnesiMotivo)HTML+='<p><i>'+htmlEntities(TXT("AnamnesiMotivo"))+':</i><br>'+htmlEntities(AnamnesiMotivo).replace(/\n/g, '<br>')+'</p>';
						if(AnamnesiDiagnosiOccidentale)HTML+='<p><i>'+htmlEntities(TXT("AnamnesiDiagnosiOccidentale"))+':</i><br>'+htmlEntities(AnamnesiDiagnosiOccidentale).replace(/\n/g, '<br>')+'</p>';
