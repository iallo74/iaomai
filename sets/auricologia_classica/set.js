...
var SET = {
	...
	_init: function(){
		...
		if(ARS){
			...
			for(a in ARS){ // aggiungo le aree
				let loader = new THREE.ObjectLoader(),
					...
					description = mesh.name.substr(name.length+2,mesh.name.length-(name.length+2)-1);
				...
				let master = (GEOMETRIE.gruppi.MASTER.punti.indexOf(name.substr(2,3))>-1);
		...
		for(let p in PTS){
			if(PTS[p]!=''){
				let x = PTS[p].array[0],
					...
					description = PTS[p].nome.substr(name.length+2,PTS[p].nome.length-(name.length+2)-1);
				let lato = "";
				...
				this.P[n].position.set(x,y,z);
				this.P[n].name=name;
				this.P[n].description=description;
				if(	(MODELLO.flip && lato == 'DX') ||
		...
		//if(DB.login.data.auths.indexOf("auricologia")!=-1){
			...
			contPulsanti += 	'>'+htmlEntities(TXT("AuricologiaModerna"))+'</option>' +
								'  <option value="_classica"';
	...
	cambiaSistema: function( sistema ){
		...
		if(DB.login.data.auths.indexOf("auricologia"+sistema)==-1){
			let select = document.getElementById("sceltaSistemaElenco1"),
				els = select.options;
			for(let e in els){
				if(els[e].value == localStorage.sistemaAuriculo)select.selectedIndex = e;
			}
			ALERT(TXT("MsgContSoloLicensed"));
			return;
		}
		localStorage.sistemaAuriculo = sistema;
		caricaSet('auricologia'+sistema);
	},
