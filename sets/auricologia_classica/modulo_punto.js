...
var MODULO_PUNTO = { // extend SET
	...
	caricaPunto: function( siglaPunto, ritorno ){ //  carica la scheda del punto
		...
		let btnAdd = 	'<div class="p_paz_ref_menu" onClick="REF.open(\'sets.auricologia_classica.pointsmap\')">' +
							TXT("ReferenceGuide") +
						'</div>';
	...
	swHidePunto: function( siglaPunto ){
		...
		for(let nota of DB.note.data){
			if(siglaPunto == nota.numeroPunto && nota.meridiano=='auriculo' &&  nota.idCL==-1 && nota.app=='AUZ'){
