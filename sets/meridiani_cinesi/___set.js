...
SET = {
	...
	_init: function(){
		...
		for(m in MERIDIANI){ // elenco i meridiani
			...
			this._PT[m] = new THREE.Group();
			this._PT[m].name="_PT_"+m;
			...
			for(p in PTS){
				if(PTS[p]!=''){
					...
					if(mouseDetect){
						...
						this.P[n] = new THREE.Points( dotGeometry, SET.MAT.dotMaterial );
						this.P[n].name=PTS[p].nome;
						if(sigla)this.P[n].userData.sigla = sigla;
						this.PT[m].add( this.P[n] );
					}
	...
	_render: function(){
	 	...
	 	if(manichinoCaricato && !raycastDisable && !controlsM._ZPR && controlsM._MM){ // roll-over sui punti
			...
			if(SETS){
				for(i in SETS.children){
					if(	SETS.children[i].isGroup && SETS.children[i].name.substr(0,2)!='LN' &&
						SETS.children[i].name.substr(0,2)!='GD' &&
						SETS.children[i].name.substr(0,2)!='PT' ){
		...
		if(this.ptSel && mouseDetect){ // pulse del pallino selezionato
			this.pulse+=0.01;
	...
	spegniMeridianoSecondario: function( sigla ){
		...
		for(m=SET.meridianiSecondariAccesi.length-1;m>-1;m--){
			if(SET.meridianiSecondariAccesi[m] == sigla || !sigla){
				...
				if(SET.meridianiSecondariAccesi[m].indexOf("_")==-1){
					...
					SET.accendiMeridiano(SET.meridianiSecondariAccesi[m],true,true);
